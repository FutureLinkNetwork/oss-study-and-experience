...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        <div>
            ...
            <div class="mt-2 text-center">
				...
				<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 mt-2">
                    <i class="fas fa-info-circle mr-1"></i>同一メールアドレスで複数登録の場合<br />すべてのログインID・パスワードをリセットします
                </span>
            </div>
        </div>
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('user.forgot-password') }}">
            @csrf
            ...
            <div>
                <button type="submit"
                    ...
                    <span class="absolute left-0 inset-y-0 flex items-center pl-3">
                        <i class="fas fa-paper-plane text-green-500 group-hover:text-green-400"></i>
                    </span>
                    パスワードリセットの案内送信
                </button>
            </div>
