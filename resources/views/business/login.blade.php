...
@section('title', '事業者マイページ ログイン - '.$subdomain->system_name)
...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('business.login') }}">
            @csrf
            <div class="rounded-md shadow-sm -space-y-px">
                <div>
                    ...
                    <input id="login_id" name="login_id" type="text" autocomplete="username" required
                           ...
                           placeholder="事業者ログインID" value="{{ old('login_id') }}">
                </div>
                <div>
        ...
        <div class="mt-6 text-center">
            <div class="mt-2">
                <a href="{{ route('business.forgot-password') }}" class="text-sm text-blue-600 hover:text-blue-500">
                    ログインID、パスワードを忘れてしまった方
                </a>
            </div>
            <div class="mt-4">
                <a href="{{ route('contact') }}" target="_blank" class="text-xs text-blue-600 hover:text-blue-500">
                    問い合わせフォーム
                </a>
            </div>
            <div class="mt-2">
                <a href="{{ route('login') }}" class="text-xs text-blue-600 hover:text-blue-500">
                    利用者の方はこちら
                </a>
            </div>
        </div>
