...
class BusinessLoginController extends Controller
{
    ...
    {
        return $this->showLoginFormWithClear($request, 'business.login');
    }
    ...
    public function login(Request $request)
    {
        ...
        $user = User::byLoginId($request->login_id, $subdomain->id)
            ...
            ->whereHas('role', function ($query) {
                $query->where('name', 'subdomain_business')
                    ->where('is_active', 1);
            })
            ->first();
        ...
        if (! $user || ! Hash::check($request->password, $user->password)) {
            throw ValidationException::withMessages([
                'login_id' => ['ログインIDもしくはパスワードが正しくありません。または事業者権限がありません。'],
            ]);
        }
    ...
    {
        return $this->performLogout($request, 'business.login');
    }
}
