...
class ExportBeneficiaryCsvMonthly extends Command
{
    protected $signature = 'app:export-beneficiary-csv-monthly';

    protected $description = '毎月1日0時時点の利用者全件CSVを生成しS3にアップロードする（サブドメイン単位）';

    public function __construct(
        protected BeneficiaryCsvExportService $csvExportService
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        $summary = $exportedAt->format('Y年n月j日 G時i分').' 利用者CSV 全件';

        $this->info("利用者CSV月次出力を開始します。出力日時: {$exportedAt->format('Y-m-d H:i:s')}");
        ...
        $this->info("利用者CSV月次出力を完了しました。処理: {$processed} 件、エラー: {$errors} 件");
    ...
    private function processSubdomain(Subdomain $subdomain, Carbon $exportedAt, string $summary): void
    {
        ...
        $filename = 'riyousha_'.$exportedAt->format('Y-m-d_His').'.csv';
        $s3Key = S3KeyPrefix::forSubdomain($subdomain->id).'/beneficiary_exports/'.$filename;
