...
class VoucherExportCsvTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_export_csv_requires_auth(): void
    {
        ...
        $response = $this->get('http://test.localhost/admin/vouchers/export-csv');
    ...
    public function test_export_csv_returns_csv_with_headers_and_data(): void
    {
        ...
        $response = $this->actingAs($this->adminUser)->get('http://test.localhost/admin/vouchers/export-csv');
        ...
        $this->assertStringContainsString('vouchers_', $response->headers->get('content-disposition'));
        ...
        $this->assertStringContainsString('クーポン番号', $decodedHeader);
        $this->assertStringContainsString('発行日', $decodedHeader);
        $this->assertStringContainsString('有効期限', $decodedHeader);
        $this->assertStringContainsString('金額', $decodedHeader);
        $this->assertStringContainsString('対象児童名', $decodedHeader);
        ...
        $this->assertStringContainsString('VOUCH-EXPORT-001', $decodedContent);
        $this->assertStringContainsString('CSV出力児童', $decodedContent);
        $this->assertStringContainsString('未使用', $decodedContent);
    ...
    public function test_export_csv_respects_search_filters(): void
    {
        ...
        $response = $this->actingAs($this->adminUser)->get(
            'http://test.localhost/admin/vouchers/export-csv?child_name='.urlencode('存在しない児童名')
        );
    ...
    public function test_export_csv_status_filter_and_labels(): void
    {
        $beneficiary = Beneficiary::factory()->create([
            'subdomain_id' => $this->subdomain->id,
            'child_name' => 'ステータス児童',
        ...
        $response = $this->actingAs($this->adminUser)->get(
            'http://test.localhost/admin/vouchers/export-csv?status=expired'
        );

        $response->assertStatus(200);
        $decoded = mb_convert_encoding($response->streamedContent(), 'UTF-8', 'SJIS-win');
        $this->assertStringContainsString('期限切れ', $decoded);
    }

    public function test_export_csv_requires_level_40_or_above(): void
    {
        ...
        $response = $this->actingAs($lowUser)->get('http://test.localhost/admin/vouchers/export-csv');
